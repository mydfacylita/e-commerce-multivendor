...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!code || !state) {
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/admin/integracao/aliexpress?error=no_code`);
    }
    ...
    if (!auth) {
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/admin/integracao/aliexpress?error=no_config`);
    }
    ...
    if (!accessToken) {
      ...
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/admin/integracao/aliexpress?error=no_token`);
    }
    ...
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/admin/integracao/aliexpress?success=authorized`);
  } catch (error) {
    console.error('❌ Erro no callback OAuth:', error);
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/admin/integracao/aliexpress?error=callback_failed`);
  }
}
