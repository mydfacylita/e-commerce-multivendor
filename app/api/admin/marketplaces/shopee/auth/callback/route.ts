...
export async function POST(request: NextRequest) {
  try {
    ...
    if (data.error) {
      console.error('Erro da API Shopee:', data);
    ...
    if (!data.access_token || !data.refresh_token) {
      return NextResponse.json(
        { error: 'Tokens não recebidos da Shopee' },
        { status: 400 }
      );
    }
    ...
    // Calcular data de expiração (tokens da Shopee expiram em 4 horas)
    ...
    console.error('Erro ao processar callback Shopee:', error);
    return NextResponse.json(
