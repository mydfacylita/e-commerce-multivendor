...
export async function POST(
  ...
  try {
    ...
    if (!order.packedAt) {
      return NextResponse.json({ error: 'Pedido precisa ser embalado primeiro' }, { status: 400 })
    }
    ...
    return NextResponse.json({ success: true, message: 'Pedido despachado com sucesso' })
  } catch (error) {
