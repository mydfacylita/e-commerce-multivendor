    ...
    const fetchData = async () => {
      try {
        ...
        setFormData({
          ...
          cashbackRate: product.cashbackRate?.toString() || '',
          attributes: (() => {
            try {
    ...
    try {
      const response = await fetch(`/api/admin/products/${params.id}`, {
        ...
        body: JSON.stringify({
          ...
          cashbackRate: formData.cashbackRate ? parseFloat(formData.cashbackRate) : null,
        }),
      })
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <h4 className="font-medium text-green-900 mb-1">💰 Cashback neste Produto</h4>
              <p className="text-xs text-green-700 mb-3">O cliente ganha esse % de volta como saldo na Conta MYD após a entrega do pedido.</p>
              <div className="flex items-center gap-3">
                <input
                  type="number" step="0.1" min="0" max="100"
                  value={formData.cashbackRate}
                  onChange={(e) => setFormData({ ...formData, cashbackRate: e.target.value })}
                  className="w-32 px-4 py-2 border-2 border-green-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-600 font-semibold text-lg"
                  placeholder="0"
                />
                <span className="text-2xl font-bold text-green-700">%</span>
                {formData.cashbackRate && parseFloat(formData.cashbackRate) > 0 && formData.price && (
                  <span className="text-sm text-green-600">
                    → Cliente ganha <strong>R$ {((parseFloat(formData.price) * parseFloat(formData.cashbackRate)) / 100).toFixed(2)}</strong> por unidade
                  </span>
                )}
              </div>
            </div>
